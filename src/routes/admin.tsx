...
import { generateEnquiryPDF } from "@/lib/pdf-report";
import { getAdminUsers, getRevenueStats, updateAdminUser, AdminUser, AdminStats, UserUpdateRequest } from "@/lib/shop-api";
            ...
            {adminMenu.map((item) => {
              ...
              return (
                <Link
                  key={item.to}
                  to={item.to}
                  className={`flex items-center gap-3 rounded-xl border px-3 py-2.5 text-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/40 hover:bg-primary/10 ${
                    isCurrent
          ...
          <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="w-full">
            ...
            <TabsContent value="reports" className="mt-6">
              <div className="space-y-4">
                <Button onClick={() => {/* PDF all orders/users */}} className="w-full">
